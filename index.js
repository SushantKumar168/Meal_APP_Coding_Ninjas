...
async function fetch_Meals_From_Api(url, value) {
...
function show_Meal_List() {
    ...
    let html = "";
    let meals = fetch_Meals_From_Api(url, inputValue);
    meals.then(data => {
            ...
            data.meals.forEach((element) => {
                ...
                <div id="card" class="card mb-3" style="width: 20rem;">
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between mt-5">
                            <button type="button" class="btn btn-warning" onclick="show_Meal_Details(${element.idMeal})">Recipe</button>
                            <button id="main${element.idMeal}" class="btn btn-outline-light active" onclick="add_Remove_To_Fav_List(${element.idMeal})" style="border-radius:50%"><i class="fa-solid fa-heart"></i></button>
                        </div>
                    </div>
                </div>
                `;
                ...
                <div id="card" class="card mb-3" style="width: 20rem;">
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between mt-5">
                            <button type="button" class="btn btn-warning" onclick="show_Meal_Details(${element.idMeal})">Recipe</button>
                            <button id="main${element.idMeal}" class="btn btn-outline-light" onclick="add_Remove_To_Fav_List(${element.idMeal})" style="border-radius:50%"><i class="fa-solid fa-heart"></i></button>
                        </div>
                    </div>
                </div>
                `;
...
async function show_Meal_Details(id) {
    ...
    let html = "";
    await fetch_Meals_From_Api(url, id).then(data => {
        html += `
...
async function show_Fav_Meal_List() {
    console.log("hello");
    ...
    if (arr.length==0) {
        ...
        for (let index = 0; index < arr.length; index++) {
            await fetch_Meals_From_Api(url,arr[index]).then(data=>{
                html += `
                <div id="card" class="card mb-3 m-2" style="width: 20rem;">
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between mt-5">
                            <button type="button" class="btn btn-warning" onclick="show_Fav_Meal_Details(${data.meals[0].idMeal})">Recipe</button>
                            <button id="main${data.meals[0].idMeal}" class="btn btn-outline-light" onclick="add_Remove_To_Fav_List(${data.meals[0].idMeal})" style="border-radius:50%"><i class="fa fa-trash" aria-hidden="true"></i>
                            </button>
                        </div>
                    </div>
                </div>
...
async function show_Fav_Meal_Details(id) {
    let url = "https://www.themealdb.com/api/json/v1/1/lookup.php?i=";
    let html = "";
    await fetch_Meals_From_Api(url, id).then(data => {
        html += `
...
function add_Remove_To_Fav_List(id) {
    let arr=JSON.parse(localStorage.getItem("favouritesList"));
    let contain=false;
    for (let index = 0; index < arr.length; index++) {
        if (id==arr[index]) {
